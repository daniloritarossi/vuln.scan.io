...
2) SINTESI IN LINGUAGGIO NATURALE (Ollama, locale): un LLM locale
   (qwen2.5:7b via http://localhost:11434) riassume il rischio a partire
   dagli id CVE realmente restituiti da OSV. Best-effort: se Ollama non e'
   raggiungibile la sintesi e' vuota e il sistema mostra solo il conteggio.
...
def _osv_raw(product: str, version: str | None, timeout: int = 15) -> dict:
    """
    ...
    except Exception as exc:  # rete assente / API down: degrada senza errori fatali
...
def query_osv(product: str, version: str | None, timeout: int = 15) -> dict:
    """
...
def query_osv_ids(product: str, version: str | None, timeout: int = 15) -> dict:
    """
...
def summarize_cves(product: str, version: str | None, ids: list[str],
                   count: int | None = None, timeout: int = 60,
                   lang: str = "en") -> str:
    """
    Sintesi del rischio via LLM locale (Ollama). Best-effort.

    'count' = totale reale di CVE da OSV (puo' superare len(ids), troncato a 10).
    'lang'  = lingua della risposta ('en' default, 'it' supportato).
    Ritorna stringa vuota se Ollama non risponde.
    """
    if not ids:
        return ""
...
def _sanitize_constraint(raw: str) -> str:
    ...
    if low in ("unknown", "none", "n/a", "sconosciuto", "nessuna"):
        return ""
    # "all versions", "every version", "tutte le versioni" -> tutte affette
    # (solo se non c'e' un vincolo numerico esplicito nel testo).
    ...
    if parts:
        return " ".join(parts)
    # Versione "nuda" senza operatore: interpreta come "affette fino a" (<=).
...
def extract_affected_version(product: str, description: str,
                             timeout: int = 60) -> str:
    """
    Estrae via LLM locale (Ollama) il RANGE di versione affetto del prodotto a
    partire da una descrizione testuale di vulnerabilita' SENZA CVE/versione.

    Ritorna un vincolo compatto (es. '<2.5.0', '>=1.0 <2.0', 'all') oppure ''
    se non determinabile o Ollama non risponde. Best-effort.
    """
    ...
    try:
        ...
        resp.raise_for_status()
        return _sanitize_constraint((resp.json().get("response") or ""))
    except Exception:
        return ""
